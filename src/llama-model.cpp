        ...
        case LLM_ARCH_QWEN3_VL:
            {
                ...
                switch (hparams.n_layer) {
                    case 36: type = hparams.n_embd / 4 == 2560 ? LLM_TYPE_4B : LLM_TYPE_8B; break;
                    case 64: type = LLM_TYPE_32B; break;
                    default: type = LLM_TYPE_UNKNOWN;
                }
            } break;
        ...
        case LLM_ARCH_QWEN3_VL:
            {
                ...
                for (int i = 0; i < n_layer; ++i) {
                    ...
                    layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd_main,   n_ff}, 0);
                    layer.ffn_down = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), {  n_ff, n_embd_main}, 0);
                    layer.ffn_up   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd_main,   n_ff}, 0);
                }
            } break;
            case LLM_ARCH_PHI2:
                {
...
void llama_model::print_info() const {
    ...
    if (arch == LLM_ARCH_QWEN3MOE || arch == LLM_ARCH_QWEN3_VL || arch == LLM_ARCH_QWEN3_VL_MOE || arch == LLM_ARCH_OPENAI_MOE) {
        LLAMA_LOG_INFO("%s: n_ff_exp         = %d\n",     __func__, hparams.n_ff_exp);
    }
